...
public class TempUIManager : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        if (ManagersMaster.Instance && ManagersMaster.Instance.Recorder)
        {
            deviceDropdown.value =
                deviceDropdown.options.FindIndex(
                    opt => opt.text == ManagersMaster.Instance.Recorder.SelectedMicDevice);
        }
    }
    ...
    {
        ManagersMaster.Instance.Recorder.SelectedMicDevice = deviceDropdown.options[index].text;
    }
